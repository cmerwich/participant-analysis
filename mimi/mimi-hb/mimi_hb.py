...
TF.load('g_prs', add=True, silent=True)
...
class Mention:
    def __init__(self, text='', start=0, end=0, node_tuple=tuple(), 
        ...
        self.pargr = ''                # paragraph number of the mention within clause_atom object
        self.txttype = ''              # text type of the mention within clause object: ?, N, D, Q
...
def OpenErrorFiles(clustername):
    ...
    for MiMi's mention detection phase.
    '''
...
def CloseErrorFile(f1):
    '''
    Close all error file objects.
    '''
...
def WriteBook(chn, filename):
    ...
    The dict is used for retrieving word indices to fill the sly Token object. 
    '''
...
def MakeTokens(chn, stats, index_dict):
    ...
    in `Mention' and added to `Token'. The tokes are appended to the list `MyTokens'. 
...
class MyParser(Parser):
    ...
    # GRAMMAR RULES FOR BIBLICAL HEBREW # 
    ...
    @_('interrogative_phrase', 'adjective_phrase', 'adverbial_phrase', 
       'complex_prepositional_phrase', 'verbal_phrase')
    def phrase_atom(self, p):
        self.statistics.rule_count[self.production.number] += 1
        #print('Mention', p[0].text, p[0].start, p[0].end)
        return p[0]
    
    @_('complex_noun_phrase')
    def phrase_atom(self, p):
        self.statistics.rule_count[self.production.number] += 1
        self.Chapter_Mentions.append(p[0])
        return p[0]
    ...
    def extended_prepositional_phrase(self, p):
        self.statistics.rule_count[self.production.number] += 1
        self.Chapter_Mentions.append(p[1])
        return p[1]
...
def MentionParseStats(stats, book_name, mention_detection_list):   
    ...
    print('\n',\
    f'Mention Detection Statistics {book_name}: \n',\
    ...
    mention_detection_list.append({'book' : book_name, 
                    'phrase atoms' : stats.pa_count, 
                    'success' : pa_success,
                    'failure' : stats.error_count,
                    '%success' : success_percent,
                    '%fail' : failure_percent
                 })
    mention_stats_df = pd.DataFrame(mention_detection_list)
    mention_stats_df = mention_stats_df[['book', 'phrase atoms', 'success', 'failure', '%success', '%fail']]
...
def find_clause(m):
    '''
    Called in ResolvePredicate(), ResolveFrontedElement(). 
...
def do_agree(mother_pred_phrase, daughter_pred_phrase):
    ...
    # in a few cases there is one only a mother word 
...
def mention_from_domain(mp, mentions):
    ...
    - has the same grammatical number 1 or 2 as mp;
...
def ResolvePronouns(mentions, coref_list, stats):
    ...
    To do:
    - 3P: Walk back e.g. to first word group for which 3P, g, n are the same)
                ...
                if resu_clause:
                    ...
                    # heuristics for if there is no phrase relation `resu'
                        ...
                        if (mfronted.rpt == 'PPrP' or mfronted.rpt == 'NP') and not mfronted.issuffix:
                            subj_pprp_phrase = find_other_phrase(resu_clause, 'Subj', 'PPrP') #Subj NP +suffix
                            ...
                            if subj_pprp_phrase:
                                subj_pprp_m = find_mention_in(subj_pprp_phrase, mentions)
                                if subj_pprp_m:
                                    
                                    Unite(coref_list, coref_list.index(mfronted.corefclass), 
                                      coref_list.index(subj_pprp_m.corefclass))

                            elif pres_vp_phrase:
                                pres_vp_mention = find_mention_in(pres_vp_phrase, mentions)
                                if pres_vp_mention:
                                    
                                    Unite(coref_list, coref_list.index(mfronted.corefclass), 
                                      coref_list.index(pres_vp_mention.corefclass))

                            elif preo_vp_phrase and not subj_pprp_phrase:
                                preo_vp_mention = find_mention_in(preo_vp_phrase, mentions)
                                if preo_vp_mention.issuffix:
                                    
                                    Unite(coref_list, coref_list.index(mfronted.corefclass), 
                                          coref_list.index(object_suffix_mention.corefclass))

                            elif prec_vp_phrase and not subj_pprp_phrase:
                                prec_vp_mention = find_mention_in(prec_vp_phrase, mentions)
                                
                                Unite(coref_list, coref_list.index(mfronted.corefclass), 
                                  coref_list.index(prec_vp_mention.corefclass))
...
def CorefResolutionStats(stats, book_name, coreference_list):
    ...
    print('\n',\
        f'Coreference Resolution Statistics {book_name}: \n',\
